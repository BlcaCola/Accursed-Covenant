...
const openChinese=async(page:import('@playwright/test').Page):Promise<void>=>{
...
test('class resource remains inside the HUD at a wide low-height resolution',async({page})=>{
  await page.setViewportSize({width:1983,height:606});await openChinese(page);await page.locator('[data-command="start"]').click();await expect(page.locator('#map-loading')).toBeHidden({timeout:20000});
...
test('HUD art and live controls share exact coordinates at 150 percent DPI',async({browser})=>{
  ...
  await openChinese(page);await page.locator('[data-command="start"]').click();await expect(page.locator('#map-loading')).toBeHidden({timeout:20_000});
...
test('mouse movement, manual cooldowns, basic attacks and corpse sprites work together',async({page})=>{
  await page.setViewportSize({width:1280,height:800});await openChinese(page);await page.locator('[data-command="start"]').click();await expect(page.locator('#map-loading')).toBeHidden({timeout:20_000});
...
test('Chinese and English UI, fogged walls and authored combat effects are active',async({page})=>{
  const errors:string[]=[];page.on('pageerror',error=>errors.push(error.message));await page.goto('/?qa=1&lang=zh');
  ...
  await page.locator('[data-command="talents"]').click();await expect(page.locator('.talent-dialog')).toBeVisible();expect((await page.locator('.talent-dialog').innerText())).not.toMatch(/[\u3400-\u9fff]/);const talentLayout=await page.evaluate(()=>{const rect=(selector:string)=>{const r=document.querySelector(selector)!.getBoundingClientRect();return{top:r.top,bottom:r.bottom,left:r.left,right:r.right}};return{dialog:rect('.talent-dialog'),header:rect('.talent-dialog>header'),canvas:rect('.talent-canvas'),footer:rect('.talent-dialog>footer'),viewport:{width:innerWidth,height:innerHeight}};});expect(talentLayout.dialog.top).toBeGreaterThanOrEqual(0);expect(talentLayout.dialog.bottom).toBeLessThanOrEqual(talentLayout.viewport.height);expect(talentLayout.header.bottom).toBeLessThanOrEqual(talentLayout.canvas.top);expect(talentLayout.canvas.bottom).toBeLessThanOrEqual(talentLayout.footer.top);await page.screenshot({path:'test-results/talent-tree-english-v16.png'});await page.locator('[data-command="talent-close"]').last().click();
...
test('every equipment rarity uses one aligned loot-beam silhouette',async({page})=>{
  await openChinese(page);await page.locator('[data-command="start"]').click();await expect(page.locator('#map-loading')).toBeHidden({timeout:20000});
...
test('exploration contracts, PNG interface, resource clipping and boss phase',async({page})=>{
  ...
  await openChinese(page);await page.locator('[data-command="start"]').click();
...
test('breakable wall, route choice and animated portal are playable',async({page})=>{
  const errors:string[]=[];page.on('pageerror',e=>errors.push(e.message));await openChinese(page);await page.locator('[data-command="start"]').click();
...
test('sealed room combat opens a three-way reward and unlocks the boss seal',async({page})=>{
  const errors:string[]=[];page.on('pageerror',error=>errors.push(error.message));await openChinese(page);await page.locator('[data-command="start"]').click();await expect(page.locator('#map-loading')).toBeHidden({timeout:20000});
...
test('developer panel controls combat telemetry without entering production UI',async({page})=>{
  const errors:string[]=[];page.on('pageerror',error=>errors.push(error.message));await openChinese(page);await page.locator('[data-command="start"]').click();
...
test('all fifteen map themes load their own geometry and room identity',async({page})=>{
  test.setTimeout(150_000);const errors:string[]=[];page.on('pageerror',error=>errors.push(error.message));await openChinese(page);await page.locator('[data-command="start"]').click();await page.locator('.qa-toggle').click();
...
test('expanded talent tree, detailed merchant, gallery code and giant final boss are playable',async({page})=>{
  test.setTimeout(120_000);const errors:string[]=[];page.on('pageerror',error=>errors.push(error.message));await openChinese(page);
