...
test('compact desktop layout keeps the start and HUD within the viewport', async ({ page }) => {
  ...
  await page.locator('[data-command="start"]').click();
  for (const id of ['#life-meter', '#mana-meter', '#inventory-button']) {
...
test('Chinese and English UI, fogged walls and authored combat effects are active',async({page})=>{
  ...
  await page.locator('.selection-language').click();await expect(page.locator('html')).toHaveAttribute('lang','en');await expect(page.locator('#game-title')).toHaveText('ASHBOUND');await expect(page.locator('.character-choice').first()).toContainText('Storm Sorceress');
  await page.locator('.selection-language').click();await expect(page.locator('html')).toHaveAttribute('lang','zh-CN');await expect(page.locator('#game-title')).toContainText('灰 烬 契 约');await page.locator('[data-command="start"]').click();await expect(page.locator('#map-loading')).toBeHidden({timeout:20000});
