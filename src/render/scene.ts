...
export class GameScene extends Phaser.Scene {
  ...
  private renderWorld(dt: number): void {
    ...
    for (const corpse of run.corpses) { const c = project(corpse),scale=corpse.rank==='boss'?2.6:corpse.rank==='superElite'?1.8:corpse.rank==='elite'?1.35:1; this.ground.fillStyle(corpse.blood??0x412722, Math.min(.58, corpse.ttl / 7)); this.ground.fillEllipse(c.x, c.y, 27*scale, 11*scale); this.ground.lineStyle(2, 0x95866c, .28); this.ground.lineBetween(c.x - 7*scale, c.y - 3, c.x + 6*scale, c.y + 3); }
