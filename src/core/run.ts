...
import { protectedItem, QUALITY_ORDER, RARITY_COLORS, rollRarity, setPieces, watcherPieces } from './equipment';
import { createItem, itemValue } from './loot';
import { clamp, distance, Random, round2 } from './random';
...
export class Run {
  ...
  constructor(readonly seed: number, readonly characterId: CharacterId, readonly session: Session,forcedTheme?:ThemeId) {
    ...
    this.reveal();
    this.dungeon.chests.unshift({ x: this.player.x - 125, y: this.player.y + 100 });
    this.setupFloorNpcs();
  ...
  update(dt: number, input: Input): void {
    ...
    if (input.burst&&!stunned) this.burst();
  ...
  private desiredMinions(skill:Skill):number{return Math.min(this.has('summonerCrown')?8:6,2+Math.floor(skill.level/2)+(skill.branch==='wide'?1:0)+(this.has('summonerCrown')?2:0));}
  ...
  smithImproveQuality(id:number):boolean{const item=this.locateItem(id);if(this.phase!=='blacksmith'||!item)return false;const current=QUALITY_ORDER.indexOf(item.rarity),cap=this.greed<10?QUALITY_ORDER.indexOf('rare'):QUALITY_ORDER.length-1;if(current<0||current>=cap)return false;const cost=this.shopPrice(100+(current+1)*75+(item.level??1)*4);if(!this.spendGold(cost))return false;const rarity=QUALITY_ORDER[current+1],improved=createItem(this.rng,item.id,item.level??item.power,rarity,undefined,item.slot,item.weaponKind);Object.assign(item,improved,{id:item.id});this.notify(`铁匠升品 · ${item.name}成为${rarity}`,'gold');return true;}
  ...
  private applyPlayerStatus(status:NonNullable<Enemy['onHit']>,potency:number,duration:number,source:string):void{
    ...
    this.events.push({type:'status',...this.player,color:{poison:0x87b85c,bleed:0xd74f54,stun:0xe8ce77,chill:0x87d4ef,curse:0xb272d8,burn:0xef713f}[status]});
  }
  ...
  private spawnEnemyProfile(profile?:MonsterProfile,forcedKind?:EnemyKind,elite=false,rankOverride?:'normal'|'elite'|'superElite'|'boss'):Enemy|undefined{
    ...
    if(rank==='boss')e.radius=profile?.artId?.startsWith('final/')?150:70;
  ...
  private cast(skill: Skill, aim?: Vec): void {
    ...
    const width = skill.branch === 'wide' ? 1.3 : 1;
    ...
    switch (skill.id) {
      ...
      case 'frost':
        ...
        for (const e of this.targets(p, 155 * width)) { e.slow = 2.8; this.hit(e, damage * 1.1, color); }
      ...
      case 'summon':
        this.raiseMinions(this.desiredMinions(skill),damage*.6,(skill.level===6?20:14)*(this.has('summonerCrown')?1.25:1));
        break;
      case 'shield': case 'warcry':
        p.shield = Math.min(120, 25 + skill.level * 12) * (skill.id === 'shield' && this.has('guard') ? 1.4 : 1);
  ...
  private raiseMinions(count: number, damage: number, duration: number): void {
    count = Math.min(this.has('summonerCrown')?8:6, count);
    ...
    while (this.minions.length < count) {
      ...
      const position = { x: this.player.x + Math.cos(angle) * 27, y: this.player.y + Math.sin(angle) * 27 };
  ...
  hit(enemy: Enemy, damage: number, color = 0xffffff, allowProc = true,source=this.damageSource): boolean {
    ...
    this.events.push({ type: executed?'execute':'hit', x: enemy.x, y: enemy.y, color, amount, critical,lethal,heavy,blocked:absorbed>=amount,impact,label:executed?'处决':undefined });
    ...
    if(allowProc&&weapon){
      this.weaponHits++;
      if(weapon==='mace'&&this.rng.next()<.12)enemy.stagger=Math.max(enemy.stagger,.65);
  ...
  private dropItem(pos: Vec, rarity: Rarity, match = false, weapon = false,themeRelic=false): void {
    ...
    const item = createItem(this.rng, this.nextId++, itemLevel, rarity, match ? this.skills[0].id : undefined, weapon ? 'weapon' : undefined);
  ...
  hurt(amount: number, source: string,attacker?:Enemy): void {
    ...
    this.events.push({type:'playerHit',...p,amount:final,color:0xee7072,blocked:absorbed>=final,impact:Math.min(1,.25+final/100)});if(attacker&&this.has('thorns'))this.hit(attacker,round2(final*.18),0xd3b17d,false);
  ...
  drinkPotion(): void {
    ...
    p.potionCharges--; p.potionCooldown = 3; p.hp = Math.min(this.stats.maxHp, p.hp + this.stats.maxHp * (.5+talentBonus(this.session,this.characterId,'potionPower')));
    this.events.push({ type: 'heal', ...p, color: 0xc45f63 }); this.notify('饮下血瓶 · 恢复 50% 生命');
  }
  burst(): void {
    const p = this.player;
    ...
    const target=this.targets(p,330)[0]; if(target){const d=Math.max(1,distance(p,target));p.attackFacing={x:(target.x-p.x)/d,y:(target.y-p.y)/d};}
    const damage = (110 + this.level * 12) * this.stats.damage;
    this.events.push({ type: 'burst', ...p, radius: this.characterId === 'bloodknight' ? 200 : 290, color: this.character.color });
    if (this.characterId === 'sorceress') {
      for (const e of this.targets(p, 290)) { this.hit(e, damage, this.character.color); e.slow = 1.5; this.events.push({ type: 'lightning', x: e.x - 20, y: e.y - 160, target: { x: e.x, y: e.y }, color: 0xb4def7 }); }
      this.projectiles = this.projectiles.filter(b => !b.enemy || distance(b, p) > 290);
    } else if (this.characterId === 'necromancer') {
      this.raiseMinions(6, damage * .2, 10);
      for (const e of this.targets(p, 180)) this.hit(e, damage * .65, this.character.color);
      const remains = this.corpses.filter(c => distance(c, p) < 300).slice(0, 6);
      for (const corpse of remains) this.explodeCorpse(corpse, damage * .6, 100);
      this.corpses = this.corpses.filter(c => !remains.includes(c));
    } else {
      const targets = this.targets(p, 200);
      for (const e of targets) { this.hit(e, damage * (e.hp / e.maxHp < .5 ? 2 : 1), this.character.color); this.pushEnemy(e, p, 220); }
  ...
  get interaction(): { label: string; kind: 'altar' | 'chest' | 'event' | 'exit' | 'wall' | 'mechanism'|'merchant'|'blacksmith'|'beggar'|'traveler'|'showcasePortal'|'showcaseExit'; index: number } | null {
    ...
    return i < 0 ? null : { label: '开启遗物宝箱', kind: 'chest', index: i };
  }
  interact(): void {
    ...
    if (newSkill) {
      ...
      for (const skill of this.skills.filter(s => s.level < 6)) {
        if (skill.level === 2 && !skill.branch) {
          const wideDescription = skill.id === 'lightning' ? '额外跳跃 2 次，单段伤害降低 15%。' : ['blood', 'bones'].includes(skill.id) ? '长矛额外穿透 3 个敌人。' : skill.id === 'summon' ? '召唤上限增加 1 名，最多 6 名。' : skill.id === 'blades' ? '环绕旋刃改为向外飞出再回返的刀刃，适合拉开距离。' : skill.id === 'lance' ? '回旋飞刃变为三路扇形投掷。' : skill.id === 'arcane' ? '秘法飞矢额外发射 2 枚。' : '技能影响半径扩大 30%。';
          options.push({ key: `wide-${skill.id}`, title: `${SKILLS[skill.id].name} · 广域`, description: wideDescription, skill: skill.id, kind: 'wide' });
          options.push({ key: `focused-${skill.id}`, title: `${SKILLS[skill.id].name} · 凝聚`, description: '升至 3 级，伤害提高 35%。链雷剩余跳跃可转为单体追加伤害。', skill: skill.id, kind: 'focused' });
        } else options.push({ key: `level-${skill.id}`, title: `${SKILLS[skill.id].name} · ${skill.level === 5 ? '终极蜕变' : '强化'}`, description: skill.level === 5 ? '升至 6 级，获得额外 40% 伤害，并强化技能规模。' : `升至 ${skill.level + 1} 级，提高基础伤害与技能效果。`, skill: skill.id, kind: 'level' });
      }
