...
export class Interface {
  ...
  constructor(private callbacks: Callbacks) {
    ...
    this.root.innerHTML = `
      ...
      <header class="topbar">
        <div class="wordmark"><span class="brand-sigil">♜</span><div>ACCURSED COVENANT<small>诅 咒 契 约</small></div><span class="build-tag">DEMO 14</span></div>
      ...
      <div class="class-resource" id="class-resource" title=""><header><b id="class-resource-name">元素共鸣</b><span id="class-resource-value">0.00 / 100.00</span></header><div><i id="class-resource-fill"></i></div></div>
  ...
  update(run: Run): void {
    ...
    const resource=run.classResource,resourceBox=this.root.querySelector<HTMLElement>('#class-resource')!;resourceBox.classList.toggle('ready',resource.ready);resourceBox.title=resource.description;text('class-resource-name',resource.name);text('class-resource-value',`${fixed2(resource.value)} / ${fixed2(resource.max)}`);(this.root.querySelector<HTMLElement>('#class-resource-fill')!).style.width=`${resource.value}%`;
  ...
  private positionItemTooltip(event:PointerEvent):void{const box=this.root.querySelector<HTMLElement>('#item-tooltip')!;if(box.hidden)return;const margin=14,width=box.offsetWidth,height=box.offsetHeight;box.style.left=`${Math.max(margin,Math.min(innerWidth-width-margin,event.clientX+18))}px`;box.style.top=`${Math.max(margin,Math.min(innerHeight-height-margin,event.clientY+18))}px`;}
