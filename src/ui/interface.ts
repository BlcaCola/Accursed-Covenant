...
export class Interface {
  ...
  update(run: Run): void {
    ...
    const resource=run.classResource,resourceBox=this.root.querySelector<HTMLElement>('#class-resource')!;resourceBox.hidden=!resource;if(resource){resourceBox.classList.toggle('ready',resource.ready);resourceBox.title=resource.description;text('class-resource-name',resource.name);text('class-resource-value',`${fixed2(resource.value)} / ${fixed2(resource.max)}`);(this.root.querySelector<HTMLElement>('#class-resource-fill')!).style.width=`${resource.value}%`;}
