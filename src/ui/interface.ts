...
import { BASE_NAMES, QUALITY_ORDER, RARITY_COLORS, SET_NAMES, WEAPONS, rarityRates, setPieces, watcherPieces } from '../core/equipment';
...
export class Interface {
  ...
  private inspectedSlot?:Item['slot'];
  ...
  constructor(private callbacks: Callbacks) {
    ...
    this.root.innerHTML = `
      ...
      <header class="topbar">
        <div class="wordmark"><span class="brand-sigil">♜</span><div>ASHBOUND<small>灰 烬 契 约</small></div><span class="build-tag">DEMO 12</span></div>
      ...
      <div class="run-info" id="run-info"><div class="clock"><span id="timer">00:00</span><small id="character-name">雷霆术士</small></div><span class="info-divider"></span><div><b id="kills">0</b><small>已放逐</small></div><div><b id="gold">0</b><small>灰烬金币</small></div></div>
      ...
      <footer class="hud" id="hud">
        ...
        <div class="orb-group mana-group"><div class="orb-label"><span>法力</span><small>MANA</small></div><div class="orb-mount"><div class="wing wing-left"></div><div class="wing wing-right"></div><div class="orb mana-orb" role="meter" aria-label="法力" aria-valuemin="0" aria-valuemax="100" id="mana-meter"><div class="orb-liquid" id="mana-liquid"><i></i></div><div class="glass"></div></div><span class="mount-jewel">◆</span></div><div class="orb-value" id="mana-value">100 <span>/ 100</span></div></div>
      </footer>
  ...
  update(run: Run): void {
    ...
    text('timer', timeText(run.time));text('kills',run.kills.toString());text('gold',fixed2(run.gold));
    ...
    const slots = run.skills.map((s, i) => `<button class="skill-slot" data-info="${s.id}" title="${SKILLS[s.id].name} · ${s.level} 级 · ${SKILLS[s.id].description}" style="--skill-color:${hex(SKILLS[s.id].color)}"><span class="slot-key">${i + 1}</span>${icon(s.id)}<span class="slot-level">${'◆'.repeat(s.level)}</span><span class="slot-name">${SKILLS[s.id].name}</span></button>`);
  ...
  private renderModal(run: Run): void {
    ...
    const key = `${run.phase}:${run.level}:${run.inventory.map(i => i.id).join(',')}:${Object.values(run.equipped).map(i => `${i.id}:${i.level}:${i.rarity}`).join(',')}:${run.greed}:${run.walletGold}:${run.amplify}:${run.graphicsQuality}:${run.selectedEncounter}:${run.lootFilter}:${this.inspectedSlot}:${run.shopOffers.map(o=>o.sold).join('')}:${run.merchantPotionStock}:${run.clearedKeyRooms}`;
    ...
    if (run.phase === 'upgrade') {
      ...
      const stats = run.stats,inspected=this.inspectedSlot?run.equipped[this.inspectedSlot]:undefined,equippedDetails=inspected?this.equippedDetails(inspected,run):'<div class="equipped-inspector empty">点击已装备物品查看完整属性与当前特殊效果。</div>';
      this.modal.innerHTML = `<section class="inventory-dialog" role="dialog" aria-modal="true" aria-labelledby="inventory-title"><header><div><span class="eyebrow">EQUIPMENT</span><h2 id="inventory-title">契约者的行装</h2></div><button class="close-button" data-command="resume" aria-label="关闭装备">×</button></header><div class="inventory-character">${run.character.name} · 当前构筑方向：<b>${run.currentBuild.name}</b> · ${run.currentBuild.description}</div><div class="set-progress">守夜者套装 ${watcherPieces(Object.values(run.equipped))} / 3 · 2件：伤害 +12% · 3件：生命 +60</div><div class="inventory-stats"><span>伤害 <b>×${stats.damage.toFixed(2)}</b></span><span>生命 <b>${fixed2(stats.maxHp)}</b></span><span>暴击 <b>${fixed2(stats.crit*100)}%</b></span></div><div class="equipment-grid">${SLOTS.map(slot => { const item = run.equipped[slot]; return `<button class="equipment-slot ${item?.rarity ?? ''} ${this.inspectedSlot===slot?'selected':''}" data-inspect-equipped="${slot}" title="${item?.description ?? '尚未装备'}">${icon(slotIcon[slot])}<div><small>${SLOT_NAMES[slot]}</small><b>${item?.name ?? '空置'}</b>${item ? this.mechanismNote(item, run) : ''}</div></button>`; }).join('')}</div>${equippedDetails}<div class="active-effects"><b>当前特殊效果</b>${this.activeEffects(run)}</div><div class="inventory-section-heading"><h3>战利品 <small>${run.inventory.length} / 24</small></h3><span>比较与换装时战斗暂停</span></div><div class="item-list">${run.inventory.length ? run.inventory.map(item => this.itemCard(item, run)).join('') : '<div class="empty-inventory">◇<h3>行囊仍然空着</h3><p>击败精英或开启地图上的金色宝箱。<br>靠近掉落即可自动拾取。</p></div>'}</div><footer><span>可用金币 ${fixed2(run.walletGold)} · 分解装备会转为本局金币</span><button class="primary" data-command="resume">返回战斗 <kbd>I</kbd></button></footer></section>`;
      ...
      this.modal.innerHTML = `<section class="dialog small-dialog" role="dialog" aria-modal="true" aria-labelledby="pause-title"><span class="eyebrow">A MOMENT OF SILENCE</span><h2 id="pause-title">圣堂，暂归寂静。</h2><p>你的远征已暂停。</p><div class="pause-help"><span><kbd>WASD / 方向键</kbd> 移动</span><span><kbd>SPACE</kbd> 闪避 · 两次充能</span><span><kbd>Q</kbd> ${run.character.burst.name} · ${run.burstCost} 法力</span><span><kbd>R</kbd> 血瓶 · 恢复半管生命</span><span><kbd>E</kbd> 开箱 / 祭坛</span><span><kbd>F</kbd> 拾取最近装备</span><span><kbd>I</kbd> 装备与分解</span><span><kbd>TAB</kbd> 展开地图</span></div><button class="toggle-row" data-command="quality"><span>画质档位<small>高清使用更清晰地形；性能档减少环境光效与活跃地块</small></span><b>${{high:'高清',standard:'标准',performance:'性能'}[run.graphicsQuality]}</b></button><button class="toggle-row" data-command="shake"><span>画面震动<small>重击停顿保持，震动强度可单独调整</small></span><b>${['关闭','轻微','标准'][run.shakeLevel]}</b></button><button class="toggle-row" data-command="amplify" aria-pressed="${run.amplify}"><span>自动法力增幅<small>自动攻击仅在有余量时消耗 12 法力</small></span><b>${run.amplify ? '开启' : '关闭'}</b></button><button class="primary" data-command="resume">继续远征 <span>→</span></button></section>`;
    } else if (run.ended) {
  ...
  private itemCard(item: Item, run: Run): string {
    const old=run.equipped[item.slot],delta=item.damage-(old?.damage??0),health=item.health-(old?.health??0),affixes=(item.affixes??[]).map(a=>`<span class="rolled-affix">${a.group==='prefix'?'前缀':'后缀'} T${a.tier} · ${a.name} +${fixed2(a.stat==='haste'||a.stat==='crit'||a.stat==='moveSpeed'||a.stat==='pickup'||a.stat==='statusResist'||a.stat==='eliteDamage'?a.value*100:a.value)}${['health'].includes(a.stat)?'': '%'}</span>`).join('');
    const weaponMechanic=item.weaponKind?`<small class="weapon-mechanic">${WEAPONS[item.weaponKind].mechanic}</small>`:'';
    return `<article class="item-card ${item.rarity}"><div class="item-art">${item.weaponKind?`<span class="equipment-art weapon-${item.weaponKind}"></span>`:icon(slotIcon[item.slot])}</div><div class="item-content"><div class="item-rarity">物品等级 ${item.level??item.power} · ${RARITY_NAMES[item.rarity]} · ${item.baseGrade?BASE_NAMES[item.baseGrade]+' · ':''}${item.weaponKind?WEAPONS[item.weaponKind].name:SLOT_NAMES[item.slot]}${item.originTheme?` · ${THEMES[item.originTheme].name}专属`:''}</div><h3>${item.name}</h3><p>${item.description}</p>${weaponMechanic}${this.mechanismNote(item,run)}<div class="rolled-affixes">${affixes}</div><div class="item-affixes"><span class="${delta>=0?'positive':'negative'}">伤害 ${delta>=0?'+':''}${fixed2(delta)}%</span><span class="${health>=0?'positive':'negative'}">生命 ${health>=0?'+':''}${fixed2(health)}</span><span>暴击变化 ${fixed2((item.crit-(old?.crit??0))*100)}%</span><span>冷却缩减变化 ${fixed2((item.haste-(old?.haste??0))*100)}%</span></div>${old?.effect?`<small class="lost-effect">替换将失去：${old.name} 的机制</small>`:''}<div class="item-actions"><button class="equip-button" data-equip="${item.id}">装备</button><button class="salvage-button" data-salvage="${item.id}">分解 +${fixed2(itemValue(item))}</button></div></div></article>`;
  }
  private click(event: MouseEvent): void {
    ...
    if(button.dataset.inspectEquipped){this.inspectedSlot=button.dataset.inspectEquipped as Item['slot'];this.modalKey='';return;}
    ...
    else if (command) { this.callbacks.command(command); if(command==='shake'||command==='quality')this.modalKey=''; }
  }
