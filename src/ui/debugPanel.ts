...
export class DebugPanel {
  ...
  constructor(private bridge:DebugPanelBridge){
    ...
    this.root.innerHTML=`<button class="qa-toggle" aria-label="开发者调试面板">QA</button><section hidden><header><b>DEMO 0.12 调试台</b><small>F2 / \`</small></header><div class="qa-readout"></div><div class="qa-grid"><select class="qa-theme" aria-label="地图主题">${themes}</select><button data-qa="load-theme">载入地图主题</button><button data-qa="next">进入下层</button><button data-qa="level">等级 +5</button><button data-qa="gold">金币 +500</button><button data-qa="greed-down">贪欲 -1</button><button data-qa="greed-up">贪欲 +1</button><button data-qa="pack">生成怪群</button><button data-qa="elite">生成精英</button><button data-qa="boss">唤醒 Boss</button><button data-qa="clear">清除怪物</button><button data-qa="item">生成装备</button><select aria-label="生成装备品质"><option value="rare">稀有</option><option value="epic">史诗</option><option value="set">套装</option><option value="unique">暗金</option><option value="legendary">传奇</option></select><button data-qa="god">无敌：关</button></div><p>仅开发环境与 ?qa=1 可见，不写入浏览器存档。</p></section>`;
  ...
  private click(event:MouseEvent):void{
    ...
    if(action==='item'){
      const rarity=(this.root.querySelector('select') as HTMLSelectElement).value as Rarity,maxId=Math.max(10000,...run.inventory.map(item=>item.id),...Object.values(run.equipped).map(item=>item.id));
